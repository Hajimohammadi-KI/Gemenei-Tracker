...
class NotesHandler(SimpleHTTPRequestHandler):
    ...
    def handle_task_log(self):
        try:
            ...
            systemDateTime = str(payload.get('systemDateTime', ''))
            createdAt = str(payload.get('createdAt', ''))

            if not day:
                self.send_json({'error': 'No day field in task log'}, status=400)
                return

            safe_name = f'tasklog_day_{day}_{uuid.uuid4().hex[:8]}.json'
            ...
            file_path.write_text(json.dumps(log, ensure_ascii=False, indent=2), encoding='utf-8')
            self.send_json({'status': 'ok', 'saved': str(file_path)})
        except Exception as exc:
            self.send_json({'error': str(exc)}, status=500)
